...
class UserController extends Controller {
	 ...
	 * @return   json
	 */
	public function actionRename($realName, $uid, $password) {
		$this->validateAdmin();
		...
		if ($uid && $password) {
			...
			$r = $user->removePasswordResetToken();
			$is_ok = true;
		}
		$this->renderJson([], $is_ok ? self::SUCCESS : self::FAIL, $is_ok ? '' : Yii::t('w', 'update failed'));
	...
	public function actionAdd() {
		...
		if ($model->load(Yii::$app->request->post())) {
			if ($user = $model->signup()) {
				...
				// 	->send();
				$user->removeEmailConfirmationToken(true);
